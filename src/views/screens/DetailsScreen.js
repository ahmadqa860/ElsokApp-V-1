    ...
    async function fetchUserInfo() {
      if (item) {
        const response = await fetch(
          `${apiUrl}/shop/productContact/${item.id}`,
        );
    ...
    <SafeAreaView style={{backgroundColor: COLORS.white}}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
            ...
            {item.images.map((i, k) => {
              return (
                <Text
                  key={k}
                  style={
                    k == active ? style.pagingActiveText : style.pagingText
                  }>
                  &#x2022;
                </Text>
              );
        ...
        <View style={style.details}>
          <View style={style.ViewContentText}>
            <Text
              style={{fontSize: 25, fontWeight: 'bold', color: COLORS.white}}>
              {item && item.product_title}
            </Text>
            <TouchableOpacity style={style.iconContainer} onPress={communityShare}>
            <Image   source={require('../../assets/core-icons/share.png')}  />
            </TouchableOpacity>
          </View>
          <View style={style.ViewContentText}>
          ...
          <View style={style.ViewContentText}>
            <Text style={style.contentText}>شرح وتفصيل:</Text>
          </View>
          <View style={style.detailsText}>
          <Text style={style.contentText}>{item.product_description}</Text>
          </View>
        </View>
      </ScrollView>
    </SafeAreaView>
